...
public class DancingLinksSolver implements SolverFactory {
    ...
    public CGrille9x9 solve(CGrille9x9 grille) {
        boolean[][] cover = initializeExactCoverBoard(grille);
        DancingLinks dlx = new DancingLinks(cover);
        dlx.runSolver(grille);
        return grille;
    }
    ...
    private boolean[][] initializeExactCoverBoard(CGrille9x9 grid) {
        boolean[][] coverBoard = createExactCoverBoard();
        for (int row = COVER_START_INDEX; row <= BOARD_SIZE; row++) {
            for (int column = COVER_START_INDEX; column <= BOARD_SIZE; column++) {
                int n = grid.get(column, row);
                if (n != NO_VALUE) {
